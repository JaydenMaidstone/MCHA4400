...
Eigen::Vector2d StateSLAM::predictFeature(const Eigen::VectorXd & x, Eigen::MatrixXd & J, const Camera & cam, std::size_t idxLandmark) const
{
    ...
    // Eigen::Vector2<autodiff::dual> fdual; // Build expression tree
    // //J = jacobian(Camera::vectorToPixel<autodiff::dual>, wrt(xdual), at(cam, xdual), fdual);
    // J = jacobian([&](const Eigen::VectorX<autodiff::dual> & x_dual) { return cam.vectorToPixel(x_dual); }, wrt(xdual), at(cam, xdual), fdual);
